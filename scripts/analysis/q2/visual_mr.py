...
import argparse
parser = argparse.ArgumentParser(description="Visualization script for mask ratio analysis")
parser.add_argument("--scale", type=str, default='20000', help="Scale of the dataset")
args = parser.parse_args()

pic_dir = f"outputs/official/q2/mr/"
...
for brief in brief_list:
    ...
    for sp in sp_list:
        ...
        for mr, group in results.items():
            if brief != '33_70b_clear':
                plt.plot(group["layer"], group["cos_sim"], marker='o', label=f"mask_ratio={mr}", linewidth=3,markersize=10)
            else:
                plt.plot(group["layer"], group["cos_sim"], marker='o', label=f"mask_ratio={mr}", linewidth=3,markersize=7)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(pic_dir, f"{brief}_sp_{sp}_cos_sim_per_layer.png"))
        pdf_output_path = os.path.join(pic_dir, f"{brief}_sp_{sp}_cos_sim_per_layer.pdf")
